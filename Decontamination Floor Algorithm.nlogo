...
to setup
  clear-all
  set-default-shape turtles "ufo top"
...
to recolor-patch  ;; patch procedure
  ...
  ifelse cancer > 0
    [ if cancer-source = 1 [ set pcolor lime ]]
...
to go
  ask turtles
  [if sum [cancer] of patches with [pcolor = lime]= 0 [ stop ]
    if color = white
    [find-swarm]
    fd robot-speed
  ]
...
to find-swarm
  ...
  [find-nearest-neighbor
    ifelse distance nearest-neighbor < 3
    [separate]
    [align]
  look-for-cancer]
end
...
to find-nearest-cancer
  set nearest-cancer min-one-of (patches in-radius 1000 with [ pcolor = lime ]) [distance myself]
  face nearest-cancer
end
...
"" ""
PENS
"food-in-pile1" 1.0 0 -13840069 true "" "plotxy ticks sum [cancer] of patches with [pcolor = lime]"
