...
to setup
  ...
  create-turtles Nanoparticles
  [ set size 3
    set color blue
    set swarm no-turtles  ]
  setup-patches
  reset-ticks
end
...
to setup-cancer ;; patch procedure
  ;; setup tumors
   if (distancexy 45 -15) = 0
  [set blood-source 1]
  if (distancexy 15 40) = 0
  [set blood-source 1]
  if (distancexy 45 15) = 0
  [set blood-source 1]
  if (distancexy 10 10) = 0
  [set blood-source 1]
  if (distancexy 25 -40) = 0
  [set blood-source 1]
  if (distancexy 40 -48) = 0
  [set blood-source 1]
  if (distancexy (0 * max-pxcor) (-0.8 * max-pycor) ) < 30
  [ set cancer-source 1 ]
  if (distancexy (-0.6 * max-pxcor) (-0.6 * max-pycor)) < 30
  [ set cancer-source 1 ]
  if (distancexy (-0.8 * max-pxcor) (0.8 * max-pycor)) < 50
  [ set cancer-source 1 ]
  if cancer-source = 1
  [ set cancer one-of [1 2 3]]
end


to recolor-patch  ;; patch procedure
  ;; give color to tumor and grid
  ifelse cancer > 0
    [ if cancer-source = 1 [ set pcolor black ]]
    [ifelse blood-source = 1 [ set pcolor red ]
    [ set pcolor orange + 3.5 ]]
end
...
to go  ;; forever button
  ask turtles
  [if sum [cancer] of patches with [pcolor = black]= 0 [ stop ]
    if color = blue
    [find-swarm]
    wiggle
    fd 1
...
to find-swarm
  set swarm other turtles in-radius Nanoparticles-vision
  if any? swarm
...
to look-for-cancer  ;; turtle procedure
  find-nearest-cancer
  if cancer > 0
  [ set cancer cancer - 100       ;; and reduce the cancer cells
    recolor-patch
    stop ]
end


to find-nearest-cancer
  set nearest-cancer min-one-of (patches in-radius cancer-vision with [ pcolor = black ]) [distance myself]
  ifelse pcolor = orange + 3.5
  [wiggle]
  [face nearest-cancer]
end

to-report average-cancer-heading
  let x-component sum[dx] of cancer
  let y-component sum[dy] of cancer
...
to find-nearest-neighbor
  set nearest-neighbor min-one-of swarm [distance myself]
end


to separate
  turn-away ([heading] of nearest-neighbor) max-align-turn
end


to align
  turn-towards average-swarm-heading max-align-turn
end


to-report average-swarm-heading
  let x-component sum[dx] of swarm
  let y-component sum[dy] of swarm
  ifelse x-component = 0 and y-component = 0
    [report heading]
    [report atan x-component y-component]
end
...
"" ""
PENS
"food-in-pile1" 1.0 0 -16777216 true "" "plotxy ticks sum [cancer] of patches with [pcolor = black]"
...
328
312
Pen down of selected Nanoparticle
...
328
364
Pen up of selected Nanoparticle
 pen-up
NIL
1
T
